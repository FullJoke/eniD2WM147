 ...
 on utilise l'attribut de l'article pour obtenir ses infos -->
				...
				<div id="detailsArticle">
					...
					<br> <br> <label id="cateArt">Categorie : <span>${ArticleAAfficher.categorie.libelle}</span>
							...
							par ${ArticleAAfficher.enchere.utilisateur.pseudo}
						</c:if>

					</label> <br> <label id="misePrixArt">Mise à prix : <span>${ArticleAAfficher.prixInitial}
							pts</span></label> <br> <fmt:parseDate
							value="${ArticleAAfficher.finEnchere}"
							pattern="yyyy-MM-dd'T'HH:mm" var="date_fin_enchere" /> <fmt:formatDate
							value="${date_fin_enchere}" pattern="dd MMMM yyyy HH:mm"
							var="dateFin" /> 
							<label id="finEncArt">Fin de l'enchère : <span>${dateFin}</span> </label>

					 <br> <label id="retraitArt">Retrait : <span>${ArticleAAfficher.retrait.rue },
